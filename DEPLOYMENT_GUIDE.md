...
注意：以上是当前直连下载模式的兼容规则，并不是文件级授权。任意已登录用户一旦获知 fileID，就可以绕过后端直接下载。不能将 fileID 难以猜测当作认证资料、发票或模型文件的安全保障。

生产上线前需要将私有文件改为私有存储 + 后端鉴权签发短期下载 URL（或者云存储可执行的逐文件访问控制），并同步适配订单、聊天及管理员认证材料下载。不要仅把 read 改成创建者可读后直接上线，否则订单双方的跨用户下载会中断。当前代码检查只收紧了后端 API 权限，尚未完成这一存储架构迁移；不要把读取规则设置成无条件 true。
规则修改后通常需要等待 1-3 分钟，再用客户和工程师两个账号分别测试。

### 2.3 配置微信手机号云调用

云托管服务需要获准调用微信“手机号快速验证”接口，否则用户同意授权后，后端仍无法用授权 `code` 换取手机号。

1. 进入当前环境 `cloud1-d8gpj5gwue506a774` 的 **云托管 → 服务 `simu-api` → 云调用**。
2. 在 **微信令牌** 中绑定当前小程序 AppID；更换 AppID 后必须在这里重新绑定。
3. 开启 **开放接口服务**。
4. 在云调用权限中添加原始接口路径：`/wxa/business/getuserphonenumber`（不要填写域名、查询参数或 `/_/` 前缀）。
5. 保存后重新部署后端版本；微信令牌可能需要几分钟同步到所有容器实例。

手机号授权 `code` 仅 5 分钟有效且只能使用一次。每次重试都应重新点击授权按钮，不能复用控制台或旧日志中的 `code`。
