...
let order, identity, existing, count, inserted, calls, deleted, publicRows, image, privateImage;
mock('../src/db', {
  parseJson: x => typeof x === 'string' ? JSON.parse(x) : x || [],
  tx: async work => work({ execute: async (sql, args) => {
    calls.push(sql);
    if (sql.includes('FROM uploaded_files')) return [[image].filter(Boolean)];
    if (sql.includes('SELECT fileId FROM identity_verification_files')) return [[privateImage].filter(Boolean)];
...
beforeEach(() => {
  ...
  image = {id:'img',uploaderId:'e',orderId:null,kind:'IMAGE',name:'case.jpg',sizeBytes:100}; privateImage = null;
...
test('公开案例字段白名单：不暴露客户、金额、原始需求或云文件', async () => {
  ...
  assert.deepEqual(Object.keys(result.items[0]).sort(), ['id', 'title', 'summary', 'imageIds', 'directions', 'softwares', 'completedMonth'].sort());
...
test('案例支持多图，拒绝超限、重复、非本人、订单附件及私有业务材料', async()=>{
  const result=await svc.saveCase('e',{...body,imageIds:['img']});
  assert.deepEqual(result.imageIds,['img']);
  assert.equal(inserted[5],'["img"]');
  for(const ids of [Array.from({length:10},(_,i)=>'f'+i),['img','img']])await assert.rejects(svc.saveCase('e',{...body,imageIds:ids}),e=>e.status===400);
  for(const patch of [{uploaderId:'other'},{orderId:'private-order'},{kind:'DOC'},{name:'evil.svg'},{sizeBytes:6*1024*1024}]){
    const original={...image};Object.assign(image,patch);
    await assert.rejects(svc.saveCase('e',{...body,imageIds:['img']}),e=>e.status===400);
    image=original;
  }
  privateImage={fileId:'img'};
  await assert.rejects(svc.saveCase('e',{...body,imageIds:['img']}),e=>e.status===400);
});
