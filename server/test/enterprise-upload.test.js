...
const sessions=new Map(),documents=new Map();
...
test('400KB资质分段重组后加密保存，重试不重复写入，读取仍限制归属',async()=>{
 ...
 assert.ok(!sessions.get('e:upload1').payload.includes(base64.slice(0,100)));
...
test('小程序每个请求小于100KB，最后一段响应丢失可重试',async()=>{
 ...
 vm.runInNewContext(fs.readFileSync(path.resolve(__dirname,'../../miniapp/utils/enterprise-documents.js'),'utf8'),sandbox);
 assert.equal((await sandbox.module.exports.upload()).id,'document');
