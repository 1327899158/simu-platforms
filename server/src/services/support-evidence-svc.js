...
async function upload(userId,b,db={query,queryOne}){
  if(b.uploadId!==undefined)return require('./enterprise-documents').uploadChunk(userId,b,upload,'support');
  ...
  const recent=await db.queryOne('SELECT COUNT(*) n FROM support_evidence_blobs WHERE userId=? AND createdAt>=DATE_SUB(UTC_TIMESTAMP(3),INTERVAL 1 DAY)',[userId]);
  ...
  await db.query('INSERT INTO support_evidence_blobs(id,userId,mime,payload,createdAt) VALUES(?,?,?,?,UTC_TIMESTAMP(3))',[id,userId,mime,payload]);return {id,name:'证据截图'+(mime==='image/png'?'.png':'.jpg')};
}
